...
class QuantityInput extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    var buy_limit = this.firstElementChild.textContent;
    var proId = this.children[1].textContent;
    var preSale = this.children[2].textContent;
    ...
    fetch("/cart.js")
      ...
      .then((data) => {
        ...
        if (count >= 2 && (buy_limit == "true" || preSale == "true")) {
          isSubmit = false;
        }
        if (
          (event.target.name === "plus" && previousValue <= 1 && isSubmit) ||
          (buy_limit == "false" && preSale == "false") ||
  ...
  updateOptions() {
    ...
    this.options = fieldsets.map((fieldset) => {
      if (count1 == 1) {
        ...
        count1++;
        return Array.from(this.querySelectorAll("input")).find(
          (radio) => radio.checked
        ).value;
      }
    });
