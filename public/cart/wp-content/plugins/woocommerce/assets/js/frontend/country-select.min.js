jQuery(function(u) {
    ...
    var t, e = wc_country_select_params.countries.replace(/&quot;/g, '"'),
        d = JSON.parse(e),
    ...
    u(document.body).on("change refresh", "select.country_to_state, input.country_to_state", function() {
        var t, e, n, o = u(this).closest(h),
            c = (o.length || (o = u(this).closest(".form-row").parent()), u(this).val()),
        ...
        d[c] ? (u.isEmptyObject(d[c]) ? (t = u('<input type="hidden" />').prop("id", s).prop("name", i).prop("placeholder", p).attr("data-input-classes", _).addClass("hidden " + _), r.hide().find(".select2-container").remove(), a.replaceWith(t)) : (e = d[c], n = u('<option value=""></option>').text(wc_country_select_params.i18n_select_state_text), p = p || wc_country_select_params.i18n_select_state_text, r.show(), a.is("input") && (t = u("<select></select>").prop("id", s).prop("name", i).data("placeholder", p).attr("data-input-classes", _).addClass("state_select " + _), a.replaceWith(t), a = o.find("#billing_state, #shipping_state, #calc_shipping_state")), a.empty().append(n), u.each(e, function(t) {
            t = u("<option></option>").prop("value", t).text(e[t]);
            a.append(t)
        }), a.val(l).trigger("change")), u(document.body).trigger("country_to_state_changed", [c, o])) : a.is('select, input[type="hidden"]') && (t = u('<input type="text" />').prop("id", s).prop("name", i).prop("placeholder", p).attr("data-input-classes", _).addClass("input-text  " + _), r.show().find(".select2-container").remove(), a.replaceWith(t), u(document.body).trigger("country_to_state_changed", [c, o])), u(document.body).trigger("country_to_state_changing", [c, o])
